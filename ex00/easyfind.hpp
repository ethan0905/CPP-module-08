...
/*   Updated: 2022/08/09 19:56:22 by esafar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    easyfind( T const & x, int nb ) {
    ...
    std::cout << CYAN "x + last = " << *(x + last) << END << std::endl;

    int exist = std::find(x, x + last, nb);
    std::cout << CYAN "Nb found? " << *std::find(x, x + last, nb) << END << std::endl;
    if (exist) {
